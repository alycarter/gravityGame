...
public abstract class Entity implements State {
	...
	public void render(Graphics g) {
		...
		g.drawOval((int)(Level.unitResolution*(location.x-(width/2))), (int)(Level.unitResolution*(location.y-(width/2))),
				(int)(Level.unitResolution*width), (int)(Level.unitResolution*width));
		g.drawLine((int)(Level.unitResolution*location.x), (int)(Level.unitResolution*location.y),
				(int)(Level.unitResolution*(location.x+(angleAsVector(direction).x*width/2))),
				(int)(Level.unitResolution*(location.y+(angleAsVector(direction).y*width/2))));
		onRender(g);
	}
